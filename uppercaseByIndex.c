...
void main(int argc,char* argv[])
{
	...
	if((in=open(argv[1],O_RDONLY))==-1)	error("cant open in file");//open input file
	if((out=open(argv[2],O_WRONLY | O_CREAT,0664))==-1)	error("cant open out file");//open output file
	...
	while(rbytes>0)//run as long we have input from file
	{
		for(i=0;i<rbytes;)
		{
			while(buff[i]!=' ' &&buff[i]!='\0')//as long the currunt char is not space or null
			{
				...
				{
					if(buff[i]>='a'&&buff[i]<='z')//if the current char is a lower letter we upper it
					{
